...
function setupIpcHandlers() {
  ...
  let currentPullModelName = null;
  electron.ipcMain.handle("ollama:pullModel", async (_event, modelName) => {
    return new Promise((resolve2, reject) => {
      var _a, _b;
      const ollamaPath = getOllamaPath();
      currentPullModelName = modelName;
      currentPullProcess = child_process.spawn(ollamaPath, ["pull", modelName], { env: ollamaEnv });
      let output = "";
      (_a = currentPullProcess.stdout) == null ? void 0 : _a.on("data", (data) => {
        const text = data.toString();
        output += text;
        mainWindow == null ? void 0 : mainWindow.webContents.send("ollama:pullProgress", { model: modelName, output: text });
      });
      (_b = currentPullProcess.stderr) == null ? void 0 : _b.on("data", (data) => {
        const text = data.toString();
        output += text;
        mainWindow == null ? void 0 : mainWindow.webContents.send("ollama:pullProgress", { model: modelName, output: text });
      });
      currentPullProcess.on("close", (code2) => {
      ...
      currentPullProcess.on("error", (error2) => {
        currentPullProcess = null;
        currentPullModelName = null;
        reject(error2);
      });
  ...
  electron.ipcMain.handle("ollama:cancelPull", async () => {
